...
class GameScene extends Phaser.Scene {
	...
	preload() {
		...
		'https://i.ibb.co/bQ1CpZd/background-FIRE.png');
		...
		this.load.spritesheet('mole',
			...
			// 'https://i.ibb.co/GpcwwS7/mole-sprite-dark.png',
			'https://i.ibb.co/VqhZt7h/mole-sprite-WARDEN.png',
    ...
    const updateScore = (points) => {
      ...
      let scoreToAdd = Math.round(5/timeElapsed);
      updateScore(scoreToAdd)
			// display how many points the user will gain
			this.displayRewardText(scoreToAdd);
	...
	initializeAnimations() {
		...
		this.anims.create({
			key: 'idle',
			frames: this.anims.generateFrameNumbers('mole', { frames: [1, 3, 1, 1, 3] }),
			frameRate: 3,
			repeat: -1,
		});
	...
	initializeTimer(timerCallback) {
		gameState.timerText = this.add.text(50, 75, `Time: ${Math.ceil(timeLeft)}`).setColor('#000000');

		this.time.addEvent({
			delay: 100, // call after every 1000ms (1sec)
	...
	updateTimerText() {
		gameState.timerText.setText(`Time: ${Math.ceil(timeLeft)}`);
	}
	...
	displayRewardText(scoreToAdd) {
		// add text to display score reward
		const rewardText = this.add.text(160, 50, `+${scoreToAdd}`).setColor('#228B22');
		this.time.addEvent({
